class GardenLevel {
    constructor(data) {
        ...
        for (var n = 0; n < types.length; n++) {
            var spacing = 370;
            var pot = new Pot();
            pot.x = 450 + n * spacing - spacing * (types.length - 1) / 2;
            pot.y = this.groundLevel;
            this.objects.unshift(pot);
        }
        for (var n = 0; n < types.length; n++) {
        ...
        this.lastPulledObject = false;
        this.maxBounceSpeed = 10;
        this.bounceEnergy = 0.5;
        this.counts = [];
    ...
    update() {
        ...
        if (Mouse.down) {
            if (this.lastPulledObject) {
                var dist = distTo(this.smoothMouse.x, this.smoothMouse.y, this.lastPulledObject.cx, this.lastPulledObject.cy);
                if (dist > 200) this.lastPulledObject = false;
                if (!o.fillAnimation && !o.waterAnimation) {
                    this.pullingObject = this.lastPulledObject;
                    this.lastPulledObject = false;
                }
            } else if (this.pullingObject) {
                ...
                var p = 1;
                if (o.delayMouseSnap) {
                    p = 1 - easeInOut(o.delayMouseSnap / 30);
                }
                o.xmove = o.xmove * (1 - p) + move.x * p;
                o.ymove = o.ymove * (1 - p) + move.y * p;
            } else if (Mouse.downStart) {
                for (var o of this.objects) {
                    if (o instanceof Pot) continue;
        ...
        this.objects = this.objects.sort((a, b) => (a.planting ? 0 : 1) - (b.planting ? 0 : 1));
        for (var o of this.objects) {
            if (o instanceof Pot) continue;
            ...
            if (o.planting) {
                ...
                o.x = o.pot.x;
                o.y = o.pot.y - o.pot.h + 33 + 20 * a1 + 25 * a2;
                if (o.plantAnimation == 60) {
                    var tree = new Tree(o.type);
                    tree.x = o.x;
                    tree.y = o.pot.y - o.pot.h + 33;
            ...
            if (o instanceof WateringCan) {
                ...
                if (o.filling) {
                    o.delayMouseSnap = 30;
                    o.x = o.x * 0.9 + (viewport.x2 - 100) * 0.1;
                    o.y = o.y * 0.9 + (700) * 0.1;
                    if (o.beingPulled) {
                        this.smoothMouse.x = o.x;
                        this.smoothMouse.y = o.y;
                    }
                    o.xmove = 0;
                    o.ymove = 0;
                    if (o.beingPulled) {
                        o.beingPulled = false;
                        this.lastPulledObject = o;
                        this.pullingObject = false;
                    }
                ...
                if (o.waterAnimation) {
                    o.delayMouseSnap = 30;
                    var o2 = o.wateringTree;
                    o.x = o.x * 0.9 + (o2.x + 130) * 0.1;
                    o.y = o.y * 0.9 + (600) * 0.1;
                    if (o.beingPulled) {
                        this.smoothMouse.x = o.x;
                        this.smoothMouse.y = o.y;
                ...
                if (o.hasWater) {
                    for (var o2 of this.objects.filter(e => e instanceof Tree)) {
                        if (o2.fruitCount >= 4) continue;
                        var dist = Math.abs(o2.x - o.x) + Math.abs(o2.y - o.cy) * 0.3;
                        if (dist > 70) continue;
                        o.waterAnimation = 50;
                        o.wateringTree = o2;
                        o.hasWater = false;
            ...
            if (o.hitbox.y < viewport.y1) {
                ...
                o.ymove = Math.min(Math.abs(o.ymove * this.bounceEnergy), this.maxBounceSpeed);
            }
            if (o.hitbox.x < viewport.x1) {
                o.hitbox = { x: viewport.x1 };
                o.xmove = Math.min(Math.abs(o.xmove * this.bounceEnergy), this.maxBounceSpeed);
            }
            ...
            if (o instanceof PlantObject && !o.unplantable) {
                var pots = this.objects.filter(e => e instanceof Pot);
                pots = pots.sort((a, b) => distTo(a.cx, a.cy, o.x, o.y) - distTo(b.cx, b.cy, o.x, o.y));
                var nearestPot = distTo(pots[0].cx, pots[0].cy, o.x, o.y);
                var nearestTree = Math.min(...this.objects.filter(e => e instanceof Tree || e.planting).map(e => Math.abs(e.x - o.x)));
                if (nearestPot < 40 && nearestTree > 100) {
                    o.planting = true;
                    o.plantAnimation = 0;
                    o.pot = pots[0];
                }
...
class Pot {
    ...
    draw() {
        ctx.drawImage(images.pot, this.x - this.w / 2, this.y - this.h, this.w, this.h);
    }
}
...
class WateringCan {
    ...
    draw() {
        ...
        ctx.scale(1.05, 1.05);
        ctx.translate(-this.w / 2, -this.h / 2);
        ctx.drawImage(images.wateringCan, -50, 0, this.w + 70, this.h);
        ctx.save();
        var waterLevel = 0.65 * this.waterLevel;
        ctx.rect(-50, this.h * (1 - waterLevel), this.w + 70, this.h * waterLevel);
        ctx.clip();
        ctx.drawImage(images.wateringCanWater, -50, 0, this.w + 70, this.h);
        ctx.restore();
        ctx.restore();
    }
}
...
class Tree {
    ...
    update() {
        this.t++;
        var growthRate = 0.05;
        var target = this.growthTarget * 0.98 + 0.02;
        this.growth = this.growth * (1 - growthRate) + target * growthRate;
    }
    grow() {
